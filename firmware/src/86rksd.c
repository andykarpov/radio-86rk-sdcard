...
/* TODO:
1) макрос GET_ADDR - читает и возвращает PINC младшие 6 bit (& 0x3F) + PINB старшие 2 bit (& 0x40)
2) макрос ROM_EMU - переводит PORTD в режим Z-state (DDRD=0, PORTD=0)

todo: алгоритм для реализации:
1) получаем адрес
2) если адрес != 0x44 - goto 1
3) вызываем ROM_EMU
4) получаем адрес
5) если адрес == 0x44 - goto 4
6) если адрес != 0x40 - goto 1
7) вызываем ROM_EMU
8) получаем адрес
9) если адрес == 0x40 goto 8
10) если адрес != 0 goto 1
*/
