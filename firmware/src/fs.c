...
typedef struct { 
  ...
  uint8_t  opened;              /* Что открыто: OPENED_NONE, OPENED_FILE, OPENED_DIR */
#endif
  uint8_t  entry_able;          /* Результат выполнения функции fs_dirread */
  WORD  entry_index;         /* Номер записи в каталоге */
  unsigned long entry_cluster;       /* Кластер записи в каталоге */
  unsigned long entry_sector;        /* Сектор записи в каталоге */
  unsigned long entry_start_cluster; /* Первый сектор файла или каталога (0 - корневой каталог FAT16) */
  unsigned long ptr;                 /* Указатель чтения/записи файла*/
  unsigned long size;                /* Размер файла / File size */
  unsigned long cluster;             /* Текущий кластер файла */
  unsigned long sector;              /* Текущий сектор файла */
  uint8_t  changed;             /* Размер файла изменился, надо сохранить */
} File;

uint8_t  fs_type;         /* FS_FAT16, FS_FAT32, FS_ERROR */
unsigned long fs_fatbase;      /* Адрес первой FAT */
unsigned long fs_fatbase2;     /* Адрес второй FAT */
uint8_t  fs_csize;        /* Размер кластера в секторах */
WORD  fs_n_rootdir;    /* Кол-во записей в корневом каталоге (только FAT16) */
unsigned long fs_n_fatent;     /* Кол-во кластеров */
unsigned long fs_dirbase;      /* Корневой каталог (сектор для FAT16, кластер для FAT32) */
unsigned long fs_database;     /* Адрес второго кластера */

/* Системные переменные. Остальное */

uint8_t  lastError;       /* Последняя ошибка */
unsigned long fs_fatoptim;     /* Первый свободный кластер */
unsigned long fs_tmp;          /* Используеются для разных целей */
...
static uint8_t fs_open0_create(uint8_t dir); /* forward */    
static CONST uint8_t* fs_open0_name(CONST uint8_t *p); /* forward */
